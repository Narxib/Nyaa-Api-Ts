(() => {
  ...
  // .wrangler/tmp/bundle-FmJ9SI/checked-fetch.js
...
`
  ...
  // .wrangler/tmp/bundle-FmJ9SI/middleware-insertion-facade.js
  ...
  __publicField(Constants, "NyaaBaseUrl", "https://nyaa.si");
  __publicField(Constants, "NyaaAltUrl", "https://nyaa.si");
  __publicField(Constants, "DefaultProfilePic", "https://raw.githubusercontent.com/Yash-Garg/Nyaa-Api-Go/dev/static/default.png");
  __publicField(Constants, "NyaaEndpoints", {
    all: {
      all: "0_0"
    },
    anime: {
  ...
  var filters = {
    contains(next2, text3, { adapter: adapter2 }) {
      return /* @__PURE__ */ __name(function contains3(elem) {
      ...
      return /* @__PURE__ */ __name(function nthChild(elem) {
        ...
        for (let i3 = 0; i3 < siblings2.length; i3++) {
          if (equals(elem, siblings2[i3]))
            break;
          if (adapter2.isTag(siblings2[i3])) {
            pos++;
          }
        }
  ...
  function setCss(el, prop2, value, idx) {
    if (typeof prop2 === "string") {
      ...
      Object.keys(prop2).forEach((k, i3) => {
  ...
  function stopParsing(p, token) {
    ...
    if (token.location) {
      ...
      for (let i3 = p.openElements.stackTop; i3 >= target; i3--) {
        p._setEndLocation(p.openElements.items[i3], token);
      }
  ...
  function listItemStartTagInBody(p, token) {
    p.framesetOk = false;
    const tn = token.tagID;
    for (let i3 = p.openElements.stackTop; i3 >= 0; i3--) {
      const elementId = p.openElements.tagIDs[i3];
      if (tn === TAG_ID.LI && elementId === TAG_ID.LI || (tn === TAG_ID.DD || tn === TAG_ID.DT) && (elementId === TAG_ID.DD || elementId === TAG_ID.DT)) {
        p.openElements.generateImpliedEndTagsWithExclusion(elementId);
        p.openElements.popUntilTagNamePopped(elementId);
        break;
      }
      if (elementId !== TAG_ID.ADDRESS && elementId !== TAG_ID.DIV && elementId !== TAG_ID.P && p._isSpecialElement(p.openElements.items[i3], elementId)) {
        break;
      }
    }
    if (p.openElements.hasInButtonScope(TAG_ID.P)) {
  ...
  function genericEndTagInBody(p, token) {
    ...
    for (let i3 = p.openElements.stackTop; i3 > 0; i3--) {
      const element = p.openElements.items[i3];
      const elementId = p.openElements.tagIDs[i3];
      if (tid === elementId && (tid !== TAG_ID.UNKNOWN || p.treeAdapter.getTagName(element) === tn)) {
        p.openElements.generateImpliedEndTagsWithExclusion(tid);
        if (p.openElements.stackTop >= i3)
          p.openElements.shortenToLength(i3);
        break;
      }
  ...
  function tokenInTableText(p, token) {
    ...
    if (p.hasNonWhitespacePendingCharacterToken) {
      for (; i3 < p.pendingCharacterTokens.length; i3++) {
        tokenInTable(p, p.pendingCharacterTokens[i3]);
      }
    } else {
      for (; i3 < p.pendingCharacterTokens.length; i3++) {
        p._insertCharacters(p.pendingCharacterTokens[i3]);
      }
    }
    p.insertionMode = p.originalInsertionMode;
    p._processToken(token);
  ...
  // src/scrapers.ts
  async function fileInfoScraper(res, url) {
    const response = await fetch(url);
    if (response.status === 200) {
      const responseBody = await response.text();
      const $2 = load(responseBody);
      const container = $2("body div.container").last();
      const fileId = Number(url.split("/")[4]);
      const torrentData = {
        title: container.find("h3.panel-title").first().text().trim(),
        file: Constants.NyaaBaseUrl + container.find("div.panel-footer a").attr("href"),
        link: `${Constants.NyaaBaseUrl}/view/${fileId}`,
        id: fileId,
        magnet: container.find("div.panel-footer a:nth-child(2)").attr("href"),
        size: container.find("div.panel-body div.row:nth-child(4) .col-md-5:nth-child(2)").text().trim(),
        category: container.find("div.panel-body div.row:nth-child(1) .col-md-5:nth-child(2)").text().trim(),
        uploaded: container.find("div.panel-body div.row:nth-child(1) .col-md-5:nth-child(4)").text().trim(),
        seeders: Number(
          container.find("div.panel-body div.row:nth-child(2) .col-md-5:nth-child(4)").text().trim()
        ),
        leechers: Number(
          container.find("div.panel-body div.row:nth-child(3) .col-md-5:nth-child(4)").text().trim()
        ),
        completed: Number(
          container.find("div.panel-body div.row:nth-child(4) .col-md-5:nth-child(4)").text().trim()
        )
      };
      ...
      let comments = [];
      if (commentCount > 0) {
        container.find("div#comments div.comment-panel div.panel-body").each((_, selection) => {
          const element = $2(selection);
          const comment = {
            name: element.find("a").first().text().trim(),
            content: element.find("div.comment-body div.comment-content").text(),
            image: element.find("img.avatar").attr("src") ?? Constants.DefaultProfilePic,
      ...
      const file = {
        ...
        submittedBy: container.find("div.panel-body div.row:nth-child(2) .col-md-5:nth-child(2)").text().trim(),
        infoHash: container.find("div.panel-body div.row:nth-child(5) .col-md-5:nth-child(2)").text().trim(),
        commentInfo: {
          count: commentCount,
          comments
        }
  ...
  __name(fileInfoScraper, "fileInfoScraper");
  async function scrapeNyaa(res, url) {
    const response = await fetch(url);
    if (response.status === 200) {
      const responseBody = await response.text();
      const $2 = load(responseBody);
  ...
  __name(scrapeNyaa, "scrapeNyaa");

  // src/utils.ts
  init_checked_fetch();
  init_middleware_insertion_facade();
  init_modules_watch_stub();
  function getCategoryID(c2, s2) {
    if (s2 === void 0) {
      return Constants.NyaaEndpoints[c2]["all"];
    } else {
      return Constants.NyaaEndpoints[c2][s2];
    }
  }
  __name(getCategoryID, "getCategoryID");
  function getSearchParameters(req) {
    const q = (req.query.get("q") ?? "").replaceAll(" ", "+");
    const p = Number(req.query.get("p"));
    const o = req.query.get("o") ?? "";
    const f2 = Number(req.query.get("f"));
    let s2 = req.query.get("s") ?? "";
    if (s2 == "date") {
      s2 = "id";
    }
    return { query: q, page: p, order: o, sort: s2, filter: f2 };
  }
  __name(getSearchParameters, "getSearchParameters");

  // src/routes.ts
  var baseUrl = Constants.NyaaAltUrl;
  var Handlers = class {
  };
  __name(Handlers, "Handlers");
  __publicField(Handlers, "Ping", function(_, res) {
    res.send(200, "Nyaa API v2 //  ALIIIIIIVE LOCALLYYY");
  });
  __publicField(Handlers, "GetInfoFromID", async function(req, res) {
    try {
  ...
  __publicField(Handlers, "GetUserUploads", async function(req, res) {
    try {
      const username = req.params.username;
      const queryParams = getSearchParameters(req);
      const searchUrl = `${baseUrl}/user/${username}?q=${queryParams.query.trim()}&p=${queryParams.page}&s=${queryParams.sort}&o=${queryParams.order}&f=${queryParams.filter}`;
      res.send(200, `URL :  ${searchUrl}`);
      await scrapeNyaa(res, searchUrl);
    } catch (error) {
      res.send(404, "this is mine");
    }
  });
  __publicField(Handlers, "GetCategoryTorrents", async function(req, res) {
    try {
      const cat = req.params.category;
      const subCat = req.params.subcategory;
      const category = getCategoryID(cat, subCat);
      const queryParams = getSearchParameters(req);
